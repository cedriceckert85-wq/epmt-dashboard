...
def rec(**kw):
    base={"phase_id":"04","finding_id":"F1","commit_sha":"abc","decision":"ACCEPT",
          "reason":"r","approver":"h","timestamp_utc":"t"}
...
def test_sha_scoping():
    ids,reasons=acceptance.accepted_ids([rec(commit_sha="old")],phase_id="04",candidate_commit="abc")
    assert ids==set() and any("stale" in r for r in reasons)

def test_other_phase_ignored_silently():
    ids,reasons=acceptance.accepted_ids([rec(phase_id="05")],phase_id="04",candidate_commit="abc")
    assert ids==set() and reasons==[]

def test_load_error_surfaces():
    ids,reasons=acceptance.accepted_ids([{"__load_error__":"x.json: boom"}],phase_id="04",candidate_commit="abc")
