"""Deterministic gate engine (V3 logic, extended for one-shot deferral).

Split (normative, from V3):
- validate_lifecycle_transition(): pure transition legality
- evaluate_completion_gate(): PASS decision only
- validate_merge_commit(): PASSED->MERGED SHA identity

V4 extension: a required test that is UNVERIFIED because of an unmet host
capability AND is listed in the phase's deferrable_tests does not block
PASS; it is recorded as deferred and reported. It is NEVER reported as a
pass. Metrics produced only by deferred tests are deferred with them.
"""
...
LEGAL_TRANSITIONS = {
    ...
    "RETESTING": {"CHECKPOINTED", "FIXING", "BLOCKED"},
    "GATE_EVALUATION": {"HUMAN_GATE", "PASSED", "FIXING", "BLOCKED"},
...
def evaluate_completion_gate(
    ...
    """Decides PASS for a phase candidate. acceptance_records are RAW
    records; the gate validates them itself against this phase and the
    exact candidate SHA."""
    reasons = []
    deferred_tests = set(deferred_tests)
    deferred_metrics = set(deferred_metrics)
    deferrable = set(phase.get("deferrable_tests", []))

    # a test may only be deferred if the phase explicitly allows it
    for t in deferred_tests:
        if t not in deferrable:
            reasons.append(f"test deferred but not deferrable: {t}")

    for test in phase.get("required_tests", []):
        if test in deferred_tests and test in deferrable:
            continue
